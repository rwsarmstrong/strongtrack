...
def drawFace(img, points, config):
    ...
    if config=='browsg':
        img = drawLinesGreen(img, brow_left, points[21])
        img = drawLinesGreen(img, brow_right, points[26])
    ...
    if config=='full':
        ...
        try:
            cv2.circle(img,(int(points[68][0]),int(points[68][1])),5,(0,255,0),-1)
            cv2.circle(img,(int(points[69][0]),int(points[69][1])),5,(0,255,0),-1)
        except:
            pass

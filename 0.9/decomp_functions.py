...
def findCoeffsAll2(points, keyposes_mouth, keyposes_brows):
...
def abstraction(coeffs, new_coeffs):
    base_coeffs = np.identity(len(coeffs))
    base_and_new_coeffs = np.concatenate(base_coeffs, new_coeffs)

    ultimate_coeffs = abstractCoeffs(coeffs, base_and_new_coeffs)

    return ultimate_coeffs


def abstractCoeffs(coeffs, profiles):
    coder = SparseCoder(dictionary=profiles, transform_n_nonzero_coefs=2,
                        transform_alpha=2, transform_algorithm='omp')

    T = np.array([coeffs, coeffs])

    coeffs = coder.transform(T)
